...
- [Navig.md](Navig.md) -- the active-navigation service (firmware Navigation,
  SID 0x07 / TCP 2054); the WPMGR/TRACK-family command/reply service -- nine
  commands (three coordinate setters plus queries) and the 54-byte
  active-navigation status report; the "active" half of the WGRT picture.
- [Dgps.md](Dgps.md) -- the differential-GPS service (firmware DGPS, SID
  0x15 / UDP); the GPS sibling on the instrument tail -- a 19-byte status
  report and a single configuration command.
...
- [Alarm.md](Alarm.md) -- the network alarm service (firmware Alarm, SID
  0x1b); fixed ports (mcast 224.0.0.2:5801 / udp 5802), broadcast by both the
  E80 and RNS -- a 15-byte event broadcast and a 9-byte control message.
